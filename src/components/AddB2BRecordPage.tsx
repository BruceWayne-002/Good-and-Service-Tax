...
const GST_REGISTRY: Record<string, string> = {
  "33FKCPS0842D1ZG": "SMS EXPORTS",
  "33AANFT5230C1ZH": "TAMIZHAN ENTERPRISES",
  "33AMPPR7163N1Z8": "RAKESHKUMAR N",
  "33BDDPT4799J1ZW": "AMUDHASURABHI RICE TRADERS",
  "33AAFCH6368A1ZU": "HANNIJI ECO POLYMERS PRIVATE LIMITED",
  "33ABCCS8325A1ZR": "SYNCDIGIT SOLUTIONS PRIVATE LIMITED",
  "33AASFV7843A1ZZ": "VIHAAN CONSTRUCTIONS",
  "33ATUPP1297M1ZU": "S.PERIASAMY CONTRACTOR",
  "33GDQPP4430Q1ZS": "NYRA LABEL",
  "33JOFPS5246A1ZZ": "ARITHABI AND CO",
};
...
const AddB2BRecordPage: React.FC = () => {
  ...
  const getMonthRange = (financialYear: string, period: string) => {
    if (!financialYear || !period) return null;
    const parts = financialYear.split(/[-–]/).map(s => s.trim());
    const startYear = Number(parts[0]);
    const monthIndex = getMonthIndex(period);
    if (!Number.isFinite(startYear) || monthIndex === null) return null;
    const actualYear = monthIndex <= 2 ? startYear + 1 : startYear;
    const firstDay = new Date(actualYear, monthIndex, 1);
    const lastDay = new Date(actualYear, monthIndex + 1, 0);
    return { min: formatYmd(firstDay), max: formatYmd(lastDay) };
  };
  const monthRange = useMemo(() => getMonthRange(fy, p), [fy, p]);
  useEffect(() => {
    if (!fy || !q || !p) {
  ...
  useEffect(() => {
    const gst = (gstin || '').toUpperCase();
    if (gst.length === 15) {
      const companyName = GST_REGISTRY[gst];
      if (companyName) {
        setRecipientName(companyName);
      }
    }
  }, [gstin]);
  ...
  useEffect(() => {
    ...
    const state = stateOptions.find((s) => s.code === code);
    if (state) {
      setPosCode(state.code);
    }
  }, [gstin]);
  ...
  const save = async () => {
    ...
    const range = getMonthRange(fy, p);
    if (!range) {
      setError('Invalid filing period.');
      return;
    }
    if (invoiceDate < range.min || invoiceDate > range.max) {
      setError('Invoice date must fall within selected filing period.');
      return;
    }
    const editId = searchParams.get('id');
    ...
    try {
      const saveInvoice = async (payload: {
        ...
        const guardRange = getMonthRange(fy, p);
        if (!guardRange) {
          throw new Error('Invalid filing period.');
        }
        if (payload.invoiceDate < guardRange.min || payload.invoiceDate > guardRange.max) {
          throw new Error('Invoice date outside selected filing period.');
        }
        if (editId) {
