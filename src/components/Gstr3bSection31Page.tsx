...
const Gstr3bSection31Page: React.FC = () => {
        ...
        ['a', 'b', 'c', 'd', 'e'].forEach(key => {
            ...
            if (savedRow?.source === 'MANUAL') {
                next[key] = {
                    ...
                    source: 'MANUAL',
                    is_tax_locked: true // Preserve the manual lock on reload
                };
            } else if (derived) {
                // Use derived (fresh GSTR-1 data)
                next[key as keyof typeof next] = derived[key as keyof typeof derived] as RowData;
            }
        });
        return next;
    });
  ...
  const setRow = (key: keyof typeof rows, field: keyof RowData, value: number) => {
    setRows(prev => {
      ...
      if (field !== "is_tax_locked") {
        r.source = "MANUAL";
      }

      if (field === "taxable" && !r.is_tax_locked) {
        const rate = 0.18;
        const halfTax = round2(value * rate / 2);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const [upsertRowsResult, existingSummaryResult] = await Promise.all([
        supabase.from('gstr3b_section_3_1').upsert(rowPayloads, { onConflict: 'user_id,filing_year,period,row_code' }),
        supabase
          .from('gstr3b_summary')
          .select('*')
          .eq('user_id', user.id)
      ...
      if (upsertRowsResult.error) {
        alert(upsertRowsResult.error.message);
        return false;
      }

      const existingSummary = existingSummaryResult.data;
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg p-6 shadow border">
          ...
          <div className="flex justify-between items-center mt-6 border-t pt-4">
            ...
            <div className="flex gap-3">
              ...
              <Button className="bg-[#1C244B] text-white" onClick={handleConfirm} disabled={saving}>
                PROCEED TO PAYMENT
              </Button>
            </div>
          </div>
