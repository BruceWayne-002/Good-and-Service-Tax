...
const Dashboard = () => {
  const { user: currentUser, company, isAdmin, signOut: logout } = useAuth();
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      const { data: ledgerBalance } = await supabase
        ...
        .select('cgst, igst, sgst, cess')
        .eq('company_email', currentUser.email)
        .eq('financial_year', '2025-26') // This should be dynamic in a real app
        .maybeSingle();

      const walletTotal = ledgerBalance 
        ? (Number(ledgerBalance.cgst || 0) + Number(ledgerBalance.igst || 0) + Number(ledgerBalance.sgst || 0) + Number(ledgerBalance.cess || 0))
        : 0;
            ...
            <div className="flex items-center gap-3">
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end" className="min-w-[12rem]">
                  ...
                  <DropdownMenuItem asChild>
                    <Link to="/returns">Return Dashboard</Link>
                  </DropdownMenuItem>
                </DropdownMenuContent>
              </DropdownMenu>
          ...
          <motion.section className="bg-white dark:bg-gray-800 rounded-lg p-6 shadow" initial="hidden" animate="visible" variants={container}>
              ...
              <div>
                ...
                {company && (
                  <div className="mt-2 text-sm">
                    <div><span className="font-medium">Company:</span> {company.trade_name}</div>
            ...
            <motion.div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mt-6" variants={item}>
              ...
              <div className="p-4 rounded-lg bg-orange-500 text-white flex flex-col items-start">
                <IndianRupee />
                <div className="mt-2 text-sm">Utilizable Cash Balance</div>
                <div className="font-bold">₹ {show(data.walletBalance?.toLocaleString('en-IN'))}</div>
              </div>
            </motion.div>
